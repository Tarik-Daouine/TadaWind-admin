...
export async function runEnrichment(client:SupabaseClient,prospectId:string,{fetchHtml=fetchPublicHtml}={}){
  ...
  if(!prospect.website)throw new Error('NO_WEBSITE')
  ...
  return {prospect:stored.data,cached:!!cached.data,url:finalUrl,signals:result.signals?.length??0}
}
