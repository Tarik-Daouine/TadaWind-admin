...
export function composeGroundedMessage(message, {channel, sources, prospectId, service}) {
  ...
  if(channel === 'phone_script') {
    variants = {phone_script:{opening:'Bonjour, je vous appelle pour Tada Wind.',reason:`Sur votre site, vous indiquez : « ${quoted} ».`,proposal,objections:[],cta}}
    grounding = [generic('phone_script.opening',variants.phone_script.opening),generic(factPath,'Sur votre site, vous indiquez : « '),factAt(factPath),generic(factPath,' ».'),offer('phone_script.proposal',proposal),generic('phone_script.cta',cta)]
  } else {
