...
export async function decrypt(value:string) {
  ...
  if(!iv||!data)throw new Error('INVALID_CIPHER')
  return new TextDecoder().decode(await crypto.subtle.decrypt({name:'AES-GCM',iv:from64(iv)},await encryptionKey(),from64(data)))
}
...
export async function outlookToken() {
  ...
  const response=await fetch('https://login.microsoftonline.com/consumers/oauth2/v2.0/token',{
    ...
    body:new URLSearchParams({client_id:Deno.env.get('MS_OAUTH_CLIENT_ID')!,client_secret:Deno.env.get('MS_OAUTH_CLIENT_SECRET')!,grant_type:'refresh_token',refresh_token:tokens.refresh_token,scope:OUTLOOK_SCOPES}),
  })
