...
> **Rédaction par canal, 2026-09-06 :** [état actuel, contrat, tests et déploiement](SINGLE-CHANNEL.md). Le worker sait produire un premier brouillon réel ; les preuves sont imbriquées puis normalisées côté serveur. Les constats sémantiques restent soumis à revue humaine. Les notes plus anciennes ci-dessous constituent l’historique.

> **Campagnes, 2026-09-06 :** vue locale raccordée à Supabase avec pagination (15 campagnes), rafraîchissement automatique, compteurs de découverte et état du job de recherche (y compris échec/retry). Les valeurs absentes restent inconnues ; une recherche terminée ne signifie pas que les traitements IA sont terminés. « Voir les prospects » applique un filtre serveur par campagne, retirable depuis la liste. Validation : 136 tests unitaires et 4 e2e réussis, 1 e2e ignoré faute de session sauvegardée, build réussi (avertissement de taille existant). Le test navigateur utilise des réponses Supabase simulées ; aucun déploiement ni campagne réelle lancé dans ce lot. Restent notamment Dashboard, Contactés, Relances et Opportunités.

> **Reprise Codex, 2026-09-06 :** changements Claude relus jusqu'à `e1a42f9`. Ajout local de `SearchCampaign.jsx`, raccordé à l'onglet Recherche et à `prospector_start_campaign` : catégories autorisées par les réglages, rayon plafonné à la limite du moteur (200 km) et de la zone, erreur explicite, verrou contre double soumission, notification du suivi des jobs. Vérifié : 136 tests unitaires, 4 e2e avec API simulée (1 ignoré faute de session locale), build réussi avec avertissement de taille du bundle. Aucun lancement de campagne réelle ni déploiement dans cette reprise. Les vues Dashboard, Contactés, Relances, Opportunités et Campagnes restent à terminer ; l'état des secrets et des déploiements indiqué ci-dessous est historique et doit être revérifié avant activation.
