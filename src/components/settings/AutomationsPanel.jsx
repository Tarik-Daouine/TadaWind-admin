...
export default function AutomationsPanel({outlookResult}) {
  const [status,setStatus]=useState(null),[outlook,setOutlook]=useState(null),[rows,setRows]=useState([]),[errors,setErrors]=useState([]),[busy,setBusy]=useState(false)
  async function load() {
    setErrors([])
    ...
    const nextErrors=[]
    if(connection.error)nextErrors.push('Le suivi Brevo est momentanément indisponible.');else setStatus(connection.data)
    if(outlookConnection.error)nextErrors.push('Le suivi Outlook est momentanément indisponible.');else setOutlook(outlookConnection.data)
    if(outbox.error)nextErrors.push("L’historique des emails est momentanément indisponible.");else setRows(outbox.data || [])
  ...
  async function connectOutlook() {
    ...
    setBusy(false)
    if(error||!data?.url){setErrors(['La connexion Microsoft n’a pas pu démarrer. Vérifie d’abord la configuration de l’application.']);return}
    const url=new URL(data.url)
    if(url.origin!=='https://login.microsoftonline.com'){setErrors(['Adresse de connexion Microsoft refusée.']);return}
  ...
  return <section style={{marginBottom:32,padding:20,border:'1px solid var(--border)',borderRadius:8}}>
    ...
    {errors.map(error=><p key={error} role="alert" style={{color:'var(--red)'}}>{error}</p>)}
    {status && <>
